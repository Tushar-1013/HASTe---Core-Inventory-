import { LayoutDashboard, Package, ArrowDownToLine, Truck, History, Settings, User, LogOut, ChevronDown, } from "lucide-react";
    ...
    return (<Sidebar collapsible="icon" className="border-r border-border">
      <SidebarContent className="pt-6">
        <div className={`px-4 mb-8 flex items-center gap-2 ${data}`}>
          ...
          {!collapsed && (<span className="font-display font-bold text-lg tracking-tighter text-foreground">
              CoreInventory
            </span>)}
        </div>
        ...
        <SidebarGroup>
          <Collapsible defaultOpen={isOpsActive}>
            ...
            <CollapsibleContent>
              <SidebarGroupContent>
                <SidebarMenu>
                  {operationsNav.map((item) => (<SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {otherNav.map((item) => (<SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <NavLink to={item.url} className="hover:bg-accent" activeClassName="bg-accent text-primary font-medium">
                      <item.icon className="mr-2 h-4 w-4"/>
                      {!collapsed && <span>{item.title}</span>}
      ...
      <SidebarFooter className="border-t border-border p-4">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild>
              <NavLink to="/profile" className="hover:bg-accent" activeClassName="bg-accent text-primary font-medium">
          ...
          <SidebarMenuItem>
            <SidebarMenuButton asChild>
              <NavLink to="/login" className="hover:bg-accent text-destructive" activeClassName="">
