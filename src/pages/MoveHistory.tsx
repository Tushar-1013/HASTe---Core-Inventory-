...
export default function MoveHistory() {
  ...
  const filtered = mockMoves.filter(
    (m) => m.product.toLowerCase().includes(search.toLowerCase()) || m.reference.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex items-center gap-2 bg-secondary rounded-md px-3 py-2 flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search moves..." value={search} onChange={(e) => setSearch(e.target.value)}
